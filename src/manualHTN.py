...
def op_wooden_axe_for_wood(state, ID):
	# Using a wooden axe is faster than punching (2 time units for 1 wood).
...
def op_craft_plank(state, ID):
	# Crafting planks consumes one wood and yields four planks.
...
def op_craft_stick(state, ID):
	# Crafting sticks consumes two planks and yields four sticks.
...
def op_craft_bench(state, ID):
	# Crafting a bench consumes four planks.
...
def wooden_axe_for_wood(state, ID):
	# Require a wooden axe before using it to gather wood.
...
state.wood = {'agent': 0}
# Assignment requirement: reach 12 wood within 46 time units.
state.time = {'agent': 46}
